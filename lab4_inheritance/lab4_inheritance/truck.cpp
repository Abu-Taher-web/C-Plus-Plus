...
void Truck::draw(PNG& canvas) const {
    // Draw all components of the truck
    trailer.draw(canvas);
    cabin.draw(canvas);
    window.draw(canvas);
    engine.draw(canvas);
...
void Truck::set_center(const Vector2& pcenter) {
    const Vector2 delta = pcenter - this->center_; // Correct delta calculation
    this->center_ = pcenter; // Set to the new center directly

    // Apply delta to all components
